          ...
          <section className="stat-card !p-6 space-y-4 border-destructive/30" aria-label={t("mandatory.title")} role="alert">
            ...
            <div className="rounded-2xl overflow-hidden border border-border">
              {mandatoryCourses.map((mc: any, index: number) => {
                ...
                return (
                  <div key={mc.id} className={`!p-3 sm:!p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 ${isOverdue ? "bg-destructive/5" : isUrgent ? "bg-orange-50/50 dark:bg-orange-950/10" : index % 2 === 0 ? "bg-card" : "bg-accent"}`} role="article" aria-label={mc.title}>
          ...
          {enrollments.length === 0 ? (
            ...
            <div className="space-y-0 rounded-2xl overflow-hidden border border-border">
              {enrollments.map((enrollment: any, index: number) => {
                ...
                return (
                  <div key={enrollment.id} className={`!p-4 sm:!p-5 space-y-3 ${index % 2 === 0 ? "bg-card" : "bg-accent"}`} role="article" aria-label={enrollment.courses?.title}>
