...
import { Search, BookOpen, Users, Clock, Sparkles, ChevronRight } from "lucide-react";
...
const CourseCatalog = () => {
  ...
  contentCounts.forEach((c: any) => {
    contentCountMap.set(c.course_id, (contentCountMap.get(c.course_id) || 0) + 1);
  });
  ...
  return (
    <DashboardLayout role="student">
      <div className="space-y-0 -m-6 lg:-m-8">
        {/* Hero banner - reusing Auth page visual concept */}
        <div className="relative h-48 lg:h-56 overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col justify-center h-full px-6 lg:px-10">
            <p className="text-xs tracking-[0.2em] text-foreground/60 uppercase mb-2">
              NONFICTION Education
            </p>
            <h1 className="font-display text-3xl lg:text-4xl tracking-wide text-foreground">
              {t("catalog.title")}
            </h1>
        ...
        <div className="px-6 lg:px-10 py-8 space-y-6">
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-2 scrollbar-hide border-b border-border">
            <button
              ...
              className={`shrink-0 px-4 py-2.5 text-sm font-medium transition-all border-b-2 -mb-[2px] ${
                activeCategory === "all"
          ...
          <div className="flex justify-center">
            <div className="relative w-full max-w-lg">
              ...
              <Input
                ...
                className="pl-11 h-12 rounded-xl border-border bg-card text-sm"
              />
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filtered.map((course: any) => {
                ...
                const studentCount = countMap.get(course.id) || 0;
                const lessons = contentCountMap.get(course.id) || 0;

                return (
                  <div key={course.id} className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
                    {/* Thumbnail */}
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {course.estimated_duration_hours != null && course.estimated_duration_hours > 0 && (
                          <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                            <Clock className="h-3 w-3" /> {t("course.duration", { hours: course.estimated_duration_hours })}
                          </span>
                        )}
                        <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                          <Users className="h-3 w-3" /> {studentCount}{t("common.people")}
                        </span>
                        {lessons > 0 && (
                          <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                            <BookOpen className="h-3 w-3" /> {t("course.lessonsCount", { count: lessons })}
                          </span>
                        )}
                      </div>
